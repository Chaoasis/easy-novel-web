<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="小说ID" prop="bookId" width="120" />
        <el-table-column align="left" label="createTime字段" prop="createTime" width="120" />
        <el-table-column align="left" label="目录名" prop="indexName" width="120" />
        <el-table-column align="left" label="目录号" prop="indexNum" width="120" />
        <el-table-column align="left" label="存储方式" prop="storageType" width="120" />
        <el-table-column align="left" label="updateTime字段" prop="updateTime" width="120" />
        <el-table-column align="left" label="字数" prop="wordCount" width="120" />
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="text" icon="edit" size="small" class="table-button" @click="updateBookIndex(scope.row)">变更</el-button>
...
export default {
  ...
  // 条件搜索前端看此方法
    ...
    async onDelete() {
      ...
      const res = await deleteBookIndexByIds({ ids })
      if (res.code === 0) {
        this.$message({
          type: 'success',
          message: '删除成功'
    ...
    async updateBookIndex(row) {
      const res = await findBookIndex({ ID: row.ID })
      this.type = 'update'
      if (res.code === 0) {
    ...
    async deleteBookIndex(row) {
      const res = await deleteBookIndex({ ID: row.ID })
      if (res.code === 0) {
        this.$message({
          type: 'success',
          message: '删除成功'
